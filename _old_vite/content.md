...
- **영상 내용 (Split Screen Video):**
    ...
    - **우측 (Galpi):** `Opt+Space` 누르고 `ㄱㅎ` 엔터! 파일이 팍! 뜨는 모습. (화면 톤: 선명하고 빠름)
...
> 
> ### **Galpi**는 당신이 찾는 모든 것을 0.5초 만에 눈앞에 가져옵니다.
> 
...
단순히 직업군을 나누는 것이 아니라, 그들이 겪는 **'구체적인 고통(Pain Point)'** 을 묘사하고, Galpi가 그것을 **'쾌감'** 으로 바꾸는 순간을 보여줍니다.
...
- **Galpi Solution (After):** *"노트와 파일을 잇는 가장 강력한 마크다운 접착제."* 
...
- **Galpi Solution (After):** *"페이지를 넘어, 문단까지 기억합니다."*
...
- **Galpi Solution (After):** *"IDE를 벗어나지 않는 맥락의 연결."* 
    > - **Intent Linking:** 코드 주석에 Galpi를 남기세요. 클릭 한 번으로 해당 코드의 **기획 의도가 담긴 슬랙 스레드**나 **피그마 시안**이 뜹니다. 
...
- The Galpi Way (After): "Galpi는 파일과 웹사이트를 '마크다운 텍스트'로 변환해 줍니다. 파인더의 파일들을 선택하고 단축키 한 번만 누르세요. 옵시디언에 붙여넣으면 끝입니다. 파일이 어디로 이동하든, 링크는 깨지지 않습니다."
- **⚡️ Killer Feature (MOC 만들기):**
    1. Galpi 런처에서 `#인공지능` 검색 (관련 논문/웹사이트 20개 검색됨)
...
- The Galpi Way (After): "파일을 여는 게 아니라, 지식을 엽니다. PDF를 읽다가 중요한 부분에 Galpi를 거세요. 나중에 링크를 클릭하면, PDF 뷰어가 켜짐과 동시에 정확히 그 페이지, 그 문단으로 이동합니다. 스크리브너(Scrivener) 집필 중에도 흐름이 끊기지 않습니다."
- **💡 Tip:** Zotero 서지 정보와 PDF 원본 파일을 Galpi로 묶어두면 연구 효율이 2배 오릅니다.
...
- The Galpi Way (After): "IDE에서 나가지 마세요. 코드 주석에 hook://ticket/123 링크를 남기세요. 클릭 한 번이면 필요한 기획서가 팝업으로 뜹니다. Electron이 아닌 Native Rust 앱이라, 당신의 소중한 메모리는 오직 컴파일러에게만 양보합니다."
...
#### Step 2. Tag (정리)
- **Copy:** **"폴더 고민 끝, 태그로 툭."**
- **Desc:** "어느 폴더에 넣지?" 고민하지 마세요. `#프로젝트A`, `#참고자료` 태그만 툭 던져두세요. 파일은 제자리에, 정리는 Galpi에.
...
- **Desc:** Galpi의 주요 로직은 **Rust**로 구성됩니다. 메모리 점유율 100MB 미만. 당신의 맥북 팬(Fan)은 이제 조용해질 겁니다.
...
### The Architecture of Galpi - ==Why we ditched the Graph DB for Sets and Rust.==
...
- **Rust Native:** Galpi는 시스템 레벨 언어인 Rust로 작성되었습니다.
...
- **Human Readable:** Galpi의 저장소는 평범한 텍스트 파일(`data.jsonl`)입니다.
...
단순히 파일 경로(Path)만 저장하는 것은 아마추어 방식입니다. Galpi는 macOS의 심층 기능을 활용해 **이중 추적 시스템**을 가동합니다.

1. **BookmarkData:** 파일 시스템의 고유 ID(Inode)를 추적합니다. 파인더에서 파일 이름을 바꾸거나 폴더를 옮겨도, Galpi는 바뀐 경로를 즉시 알아냅니다.
...
> **"Galpi는 당신의 데이터를 암호화된 DB에 가두지 않습니다. 당신의 모든 기록은 평범한 텍스트 파일로 저장되며, 당신이 100% 소유합니다. 그런데도 훅마크보다 빠르죠. Rust니까요."**
...
> **"Hookmark는 동기화하다가 DB가 깨질 수 있습니다. Galpi는 절대 깨지지 않습니다. 혹시 충돌이 나도, 우리가 알아서 합쳐드립니다."**
...
- **Galpi의 대처법: "능동적 건강 검진 (Proactive Health Check)"**
...
- **Galpi의 대처법: "자체 튜토리얼 데이터 (Playground)"**
    - 앱을 설치하자마자 `data.jsonl`이 비어있지 않게 하세요.
    - **기본 데이터:** "👋 Galpi 사용법.pdf", "🔗 이 링크를 눌러보세요(웹사이트)" 같은 예제 데이터를 미리 3~4개 넣어두세요.
...
- **Galpi의 대처법: "스마트 에이징 (Smart Aging)"**
...
- **Sandbox 문제:** 선생님 앱(Galpi)은 다른 앱의 파일 경로를 추적하고, `xattr`를 쓰고, 창을 제어해야 합니다. 이 기능들은 Apple의 **App Store 샌드박스 정책(App Sandbox)**과 정면으로 충돌합니다.
...
- 서브 헤드라인:
    ...
    Galpi는 이 모든 문제를 아키텍처 레벨에서 해결했습니다.

### 2. The Truth Table (강화된 비교표)

단순히 기능 유무(O/X)가 아니라, **'어떤 기술을 썼는지'** 를 명시하여 신뢰도를 높입니다.

| **비교 항목**  | **기존 연결 도구 (Competitor H)**  | **Galpi (차세대 엔진)**        | **비고 (Why?)**   |
| ---------- | ---------------------------- | ---------------------------- | --------------- |
...
> Galpi는 Rust로 작성된 네이티브 엔진입니다. 스크립트를 거치지 않고 OS와 직접 통신하므로, 딜레이가 구조적으로 발생할 수 없습니다.

#### Fact 2. 선 긋기(Wiring) vs 담기(Grouping)

> "일일이 연결하느라 시간 낭비하지 마세요."
> 파일 A와 B를 연결하고, B와 C를 연결하는 1:1 방식(Graph)은 파일이 늘어날수록 관리가 불가능해집니다.
> Galpi는 '태그 집합(Set)'을 사용합니다. #Project_A 태그 하나만 달면, 그 안의 100개 파일이 즉시 상호 연결됩니다. 복잡한 선 긋기 노동에서 해방되세요.

#### Fact 3. 블랙박스(Blackbox) vs 텍스트(Text)

> "당신의 데이터는 당신이 읽을 수 있어야 합니다."
> 알 수 없는 바이너리 DB에 데이터를 가두지 마세요. DB가 깨지면 복구할 수 없습니다.
> Galpi는 모든 데이터를 '텍스트 파일(JSONL)'로 저장합니다. 메모장으로 열 수 있고, 깃(Git)으로 백업할 수 있으며, 앱이 없어져도 데이터는 온전히 당신의 것입니다.
...
- **Our Solution (Rust):** 우리는 타협하지 않았습니다. Galpi는 시스템 프로그래밍 언어인 **Rust**로 작성되었습니다. 스크립트를 거치지 않고 macOS의 Accessibility API와 직접 통신합니다.
...
- **Plain Text Storage:** Galpi의 모든 데이터는 당신의 로컬 폴더에 `JSONL` 형식의 텍스트 파일로 저장됩니다.
...
Galpi는 **단 한 번의 구매(One-time Purchase)** 를 지향합니다. 커피 한 잔 값으로, 이 도구는 영원히 당신의 것이 됩니다.
...
## Galpi 가격 및 라이선스 전략 기획서 (Ver 1.0)
...
- **논리적 근거:**
    - **습관 형성:** 50개의 링크를 만들 때쯤이면 사용자는 이미 Galpi에 중독(Lock-in)되어 있습니다. 이때 결제창을 띄우면 전환율이 가장 높습니다.
...
- **Local-First:** Galpi는 클라우드 서버를 쓰지 않습니다. 모든 데이터는 사용자 로컬(JSONL)에 저장됩니다. 9
...
- **배포:** 홈페이지에서 DMG 파일 직접 다운로드.
    - **이유:** **샌드박싱(Sandboxing) 회피.** Galpi의 핵심 기능(파일 추적, 딥링크, 창 제어)은 App Store 샌드박스 환경에서는 구현 불가능합니다.
...
- A. 레이캐스트는 '도구'를 열지만, Galpi는 '작업'을 엽니다.
    레이캐스트는 PDF 뷰어를 실행해 줄 뿐입니다. 하지만 Galpi는 그 PDF의 14페이지, 세 번째 문단을 바로 열어줍니다.
    레이캐스트는 슬랙을 켜주지만, Galpi는 **'기획팀 채널의 특정 스레드'** 로 바로 이동합니다.
    Raycast와 함께 쓰세요. 앱 실행은 Raycast로, 맥락 연결은 Galpi로. 둘은 최고의 콤비입니다.

#### 2. 지속 가능성 방어 (Sustainability)

- **Q. 구독료도 없이 '평생 소장'이라니, 나중에 업데이트 중단되는 거 아닌가요?**
- A. 우리는 서버 비용이 '0원'인 구조를 만들었습니다.
    대부분의 앱이 구독을 받는 이유는 클라우드 서버 유지비 때문입니다. 하지만 Galpi는 모든 데이터를 **당신의 컴퓨터(Local)** 에서 처리합니다.
    우리에겐 매달 나가는 고정비가 없습니다. 따라서 한 번의 판매로도 충분히 지속 가능한 비즈니스 모델입니다. 안심하고 평생 쓰세요.

#### 3. 데이터 주권 방어 (No Lock-in)

- **Q. 만약 제가 Galpi를 더 이상 안 쓰게 되면, 저장해둔 링크는 다 사라지나요?**
- A. 절대 사라지지 않습니다. 데이터는 100% 당신의 것입니다.
    Galpi는 알 수 없는 암호화 DB를 쓰지 않습니다. 모든 링크 정보는 Documents/Galpi/data.jsonl이라는 투명한 텍스트 파일에 저장됩니다.
...
- A. 7일간 충분히 써보시고 결정하세요.
    ...
    우리는 당신이 억지로 이 앱을 쓰길 원하지 않습니다. 정말 도움이 될 때만 Galpi를 소장하세요.
