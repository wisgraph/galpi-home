...
**Galpi는 다릅니다. 우리는 앱이 아프기 전에 미리 진단하고 처방합니다.**
...
## 2. Galpi의 해결책: "능동적 건강 검진 (Proactive Health Check)"

Galpi는 앱이 실행되는 순간, 그리고 백그라운드에서 주기적으로 **자신의 건강 상태를 스스로 진단**합니다.
...
*   **원클릭 복구 (One-Click Fix):** "복구하기" 버튼을 누르면, 사용자가 시스템 설정을 찾아 헤맬 필요 없이, Galpi가 정확한 설정 창을 띄워주고 체크를 유도합니다.
...
*   **Automation Privilege Probe:** '자동화 권한'은 확인하기 까다롭지만, Galpi는 Finder에게 무해한 더미 명령(`tell application "Finder" to get name of startup disk`)을 보내 권한 유무를 정확히 판별해냅니다.

## 요약

| 기능 | 기존 앱 (Hookmark 등) | Galpi |
| :--- | :--- | :--- |
| **진단 시점** | 기능 실행 후 (에러 발생 시) | **기능 실행 전 (사전 예방)** |
| **에러 처리** | "권한이 없습니다" (사용자 책임) | **"이 버튼을 누르면 해결됩니다" (앱 책임)** |
| **성능** | 느린 스크립트 실행 | **Rust 기반 초고속 네이티브 호출** |

**Galpi는 당신에게 "안 되는 이유"를 설명하지 않습니다. 그냥 "되게" 만듭니다.**
