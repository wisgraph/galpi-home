        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-white truncate">{t('howItWorks.steps.2.fileName') || '2026_기획서_vFinal.pdf'}</span>
              <span className="px-1.5 py-0.5 rounded text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-500">PDF</span>
            </div>
            <div className="text-xs text-slate-400 truncate">/Documents/Work/Galpi/...</div>
          </div>
        </div>
      </motion.div>
    </div>
...
const HowItWorks: React.FC = () => {
  ...
  return (
    <section id="how-it-works" className="py-24 bg-slate-950 border-t border-slate-800/50 relative">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {steps.map((step, idx) => (
            <div key={idx} className="relative group">
              ...
              <div className="relative bg-slate-950/80 border border-slate-800 rounded-3xl p-8 h-full flex flex-col backdrop-blur-sm hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                ...
                {/* Animation Container */}
                <div className="mt-auto bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-800 h-40 w-full relative overflow-hidden group-hover:border-slate-300 dark:group-hover:border-slate-700 transition-colors">
                  <step.animation />
                </div>
              </div>
            </div>
