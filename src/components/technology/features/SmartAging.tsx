        ...
        <div className="min-h-screen bg-slate-50 dark:bg-black font-sans selection:bg-orange-500/30">
            ...
            <main className="max-w-4xl mx-auto px-6 pb-32 relative z-10 space-y-24">
                ...
                <section className="text-center max-w-2xl mx-auto">
                    <h2 className="text-2xl font-bold text-white mb-4">링크가 쌓였을 때의 문제점 (Link Clutter)</h2>
                    <p className="text-lg text-slate-400 leading-relaxed">
                        1년 전에 끝난 프로젝트 파일이 검색창 맨 위에 뜨시나요?
                        <br />
                        Galpi는 검색 엔진에 <strong>"시간의 개념"</strong>을 도입하여 이 문제를 해결했습니다.
                    </p>
                </section>

                {/* Mechanism */}
                <section className="bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-12 shadow-xl">
                    <div className="text-center mb-10">
                        <div className="inline-block bg-slate-800 px-6 py-3 rounded-xl font-mono text-sm md:text-lg text-slate-300 border border-slate-200 dark:border-slate-700 shadow-inner">
                    ...
                    <div className="grid md:grid-cols-2 gap-12 relative">
                        ...
                        <div className="text-center space-y-4">
                            ...
                            <h3 className="text-xl font-bold text-white">최신성 부스트 (Recency Boost)</h3>
                        ...
                        <div className="text-center space-y-4">
                            ...
                            <h3 className="text-xl font-bold text-white">자연스러운 감쇠 (Natural Decay)</h3>
                            <p className="text-sm text-slate-400 leading-relaxed px-4">
                ...
                <section>
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <h2 className="text-3xl font-bold text-white">언제나 신선한 이유 (Feels Fresh)</h2>
                    </div>

                    <div className="grid md:grid-cols-3 gap-6">
                        <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800">
                            <Trash2 className="w-8 h-8 text-slate-400 mb-4" />
                            <h4 className="font-bold text-white mb-2">자동 청소</h4>
                            <p className="text-sm text-slate-400">1년 전 자료가 검색창을 어지럽히지 않도록 자동으로 하단으로 밀어냅니다.</p>
                        </div>
                        <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800">
                            <Zap className="w-8 h-8 text-orange-500 mb-4" />
                            <h4 className="font-bold text-white mb-2">즉각적 접근</h4>
                            <p className="text-sm text-slate-400">방금 닫은 파일을 다시 열 때, 단축키 한 번이면 100% 확률로 맨 위에 있습니다.</p>
                        </div>
                        <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800">
                            <RefreshCcw className="w-8 h-8 text-emerald-500 mb-4" />
                            <h4 className="font-bold text-white mb-2">유지보수 제로</h4>
                            <p className="text-sm text-slate-400">별도 '아카이브' 버튼을 누를 필요가 없습니다. 그냥 쓰면 알아서 정리됩니다.</p>
                        </div>
                    </div>
                </section>
                ...
                <section className="overflow-hidden rounded-2xl border border-slate-800">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 dark:bg-slate-900 text-slate-400 font-medium uppercase text-xs">
                            <tr>
                                <th className="px-6 py-4">기능</th>
                                <th className="px-6 py-4">기존 런처</th>
                                <th className="px-6 py-4 text-orange-600 dark:text-orange-400">Galpi</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            <tr className="bg-white dark:bg-black">
                                <td className="px-6 py-4 font-medium text-white">랭킹 기준</td>
                                <td className="px-6 py-4 text-slate-500">단순 텍스트 매칭</td>
                                <td className="px-6 py-4 font-bold text-white">텍스트 + 시간 (Time Factor)</td>
                            </tr>
                            <tr className="bg-white dark:bg-black">
                                <td className="px-6 py-4 font-medium text-white">오래된 파일</td>
                                <td className="px-6 py-4 text-slate-500">여전히 상단 노출 (방해됨)</td>
                                <td className="px-6 py-4 font-bold text-white">알아서 하단으로 침전</td>
                            </tr>
                            <tr className="bg-white dark:bg-black">
                                <td className="px-6 py-4 font-medium text-white">관리 방식</td>
                                <td className="px-6 py-4 text-slate-500">수동 아카이브 필요</td>
                                <td className="px-6 py-4 font-bold text-white">자동 (Automatic)</td>
                            </tr>
                        </tbody>
                    </table>
                </section>
