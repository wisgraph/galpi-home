# Galpi Data Architecture: Transparency, Speed, and Safety

Galpi는 사용자의 데이터를 블랙박스(SQLite, Binary DB)에 가두지 않습니다.
우리는 **"Plain Text"**의 힘을 믿으며, 누구나 읽을 수 있고 어디서나 동기화 가능한 아키텍처를 설계했습니다.

이 문서는 Galpi가 어떻게 데이터를 저장하고, 보호하며, 클라우드 환경에서 충돌 없이 동기화하는지를 설명하는 기술 백서입니다.
...
여러 개의 Galpi 인스턴스를 켜도, 하나의 프로세스만이 쓰기 권한을 가지며 데이터가 뒤섞이는 것을 원천 차단합니다.

## 4. Cloud Sync & Conflict Resolution

Dropbox, iCloud, Google Drive를 통해 데이터를 동기화할 때 가장 큰 문제는 **"충돌(Conflict)"**입니다.
Galpi는 충돌을 두려워하지 않고 포용하는 **"Auto-Merge"** 전략을 사용합니다.
...
Galpi는 Rust의 강력한 성능을 활용하여 **In-Memory** 아키텍처로 동작합니다.
...
| Feature | Legacy Apps (Hookmark etc.) | Galpi |
| :--- | :--- | :--- |
...
Galpi is not just a tool; it's a commitment to **Data Sovereignty** and **Engineering Excellence**.
